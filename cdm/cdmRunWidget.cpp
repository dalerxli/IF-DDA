...
void cdmlibwrapper(Options *options, Run *run, QString *infoMessage, int *stopFlag) 
{
    ...
    for(int i = 0; i < 64; i++)
     beam[i] = ' ';
    strncpy(beam,(char*)options->getBeam().toStdString().c_str(),options->getBeam().size());
    ...
    for(int i = 0; i < 64; i++)
     object[i] = ' ';
    strncpy(object,(char*)options->getObject().toStdString().c_str(),options->getObject().size());
    ...
    if ( dipolepsilon == true )
      ...
      nobjet = false;
    int nxm,nym,nzm,ntheta,nphi;
    nxm = options->getNxm();
    nym = options->getNym();
    nzm = options->getNzm();
    ntheta = options->getNtheta();
    nphi = options->getNphi();
    QLOG_INFO() << "NMAX:" << nxm*nym*nzm << "NTHETA:" << ntheta << "NPHI:" << nphi;
    int nfft2d;
   ...
   double ss;
   if (options->getBeam() == "pwavelinear")
     ss = options->getPolarizationTE();
   else if (options->getBeam() == "pwavecircular")
...
//****************************************************
   ...
   cdmlib_(&wavelength, beam, object, anisotropy, material,
	   &discretization, &tolerance, methodeit, polarizability, &quad, &nreadCheck, filereread, &nmatlabCheck,
...
RunWidget::displayResults()
{
   ...
   farfieldComboBox->addItems((QStringList() << "Scattered field" <<  "Scattered field+Incident" << "Image" << "Image+Incident"));
   ...
   farfieldGroupBox->setTitle("Far field investigation");
   ...
   if ( options->getNearfield() || options->getFarfield() ||  
	options->getForce() || options->getDipolepsilon())
...
RunWidget::plotmicroscopy() {
   ...
   if ( type == "Modulus" ) {
     ...
     else if ( field == "Scattered field+Incident" ) {
       title = "Scattered field+Incident";
       xtitle = "k_x/k_0";
       ytitle = "k_y/k_0";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
	  data->push_back(QwtPoint3D(run->getKXY()[line],run->getKXY()[col], sqrt(
     ...
     else if ( field == "Scattered field" && type == "x" ) {
       title = "Scattered field X";
       xtitle = "k_x/k_0";
       ytitle = "k_y/k_0";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Scattered field+Incident" && type == "x" ) {
       title = "Scattered field +Incident X";
       xtitle = "k_x/k_0";
       ytitle = "k_y/k_0";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Image+Incident" && type == "x" ) {
       title = "Image+Incident plane X";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Scattered field" && type == "y" ) {
       title = "Scattered field Y";
       xtitle = "k_x/k_0";
       ytitle = "k_y/k_0";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Scattered field+Incident" && type == "y" ) {
       title = "Scattered field+Incident Y";
       xtitle = "k_x/k_0";
       ytitle = "k_y/k_0";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Image+Incident" && type == "y" ) {
       title = "Image+Incident plane Y";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Scattered field" && type == "z" ) {
       title = "Scattered field Z";
       xtitle = "k_x/k_0";
       ytitle = "k_y/k_0";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Scattered field+Incident" && type == "z" ) {
       title = "Scattered field+Incident Z";
       xtitle = "k_x/k_0";
       ytitle = "k_y/k_0";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
     ...
     else if ( field == "Image+Incident" && type == "z" ) {
       title = "Image+Incident plane Z";
       xtitle = "x(m)";
       ytitle = "y(m)";
       for ( int i = 0 ; i < nfft2d*nfft2d ; i++ ) {
...
RunWidget::plotxforce() {
   ...
   QLOG_INFO() << "RunWidget::plotxforce> plotting" << field;
}
void
...
RunWidget::plotx(QString field, double xvalue) {
    QLOG_INFO() << "RunWidget::plotx> " << field << " " << QString::number(xvalue);
    plotAxislist(field,
	run->getYc(),run->getZc(),run->getXc(),
	run->getYcWF(),run->getZcWF(),run->getXcWF(),
...
RunWidget::dipoles3Dplot() {
  ...
  int nmax = options->getNym() * options->getNym() * options->getNzm();
  QLOG_INFO() << " getObjectSubunits = " << run->getObjectSubunits();
  QLOG_INFO() << " nmax = " << nmax;
  /*QFile filex("xc.txt");
  QFile filey("yc.txt");
  QFile filez("zc.txt");
