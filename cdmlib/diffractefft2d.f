...
!$OMP DO  SCHEDULE(STATIC) COLLAPSE(2)  
      do i=-imax,imax
         do j=-imax,imax
            ii=imax+i+1
            jj=imax+j+1
            Ediffkzpos(ii,jj,1)=Ediffkzpos(ii,jj,1)*fac
            Ediffkzpos(ii,jj,2)=Ediffkzpos(ii,jj,2)*fac
            Ediffkzpos(ii,jj,3)=Ediffkzpos(ii,jj,3)*fac
            Ediffkzneg(ii,jj,1)=Ediffkzneg(ii,jj,1)*fac
            Ediffkzneg(ii,jj,2)=Ediffkzneg(ii,jj,2)*fac
            Ediffkzneg(ii,jj,3)=Ediffkzneg(ii,jj,3)*fac
c            write(*,*) Ediffkzpos(ii,jj,1), Ediffkzpos(ii,jj,2)
c     $           ,Ediffkzpos(ii,jj,3),Ediffkzneg(ii,jj,1),Ediffkzneg(ii
c     $           ,jj,2),Ediffkzneg(ii,jj,3),ii,jj
         enddo
      enddo
!$OMP ENDDO 
!$OMP END PARALLEL
