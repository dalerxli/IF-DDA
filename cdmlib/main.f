...
c     variable pour avoir l'image a travers la lentille
      integer nlentille,nobjet,nfft2d,nfft2d2,nquicklens
...
c         epsmulti(2)=(1.25d0,0.d0)        
      ...
      elseif (object(1:9).eq.'arbitrary') then
         namefileobj='arbitrary.in'
c     definir ici l'objet arbitraire
      endif
      if (numberobjet.ne.numberobjetmax) then
...
c     output
     ...
     $        ,MIECSCA,GSCA)
      ...
      write(*,*) 'essai',1.d0/3.d0/xgaus*(k0**4.d0)*quatpieps0
     $     *(epsmulti(1)-1.d0)/(epsmulti(1)+2.d0)*(rayonmulti(1)**3.d0
     $     *1.d-27)
      end
