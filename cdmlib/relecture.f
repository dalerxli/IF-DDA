      subroutine relecture(lambda,nrig,nfft2,beam,object,trope,nnnr
     $     ,tolinit,
...
c     planewavecircular.in / planewavelinear.in files
     ...
     $     ,nquad,filereread,nlecture,nstop,infostr)
      implicit none
      integer ntmp,ierror,test,ii,jj,k,nlecture,nnnr,numberobjet
     $     ,numberobjetmax,nstop,long,long1,nquad,ng,nrig,nfft2
...
c     le fichier n'a pas encore ete cree et on le cree
      if (ierror.ne.0) then
         ...
         elseif (beam(1:14).eq.'gfftwavelinear') then
            ...
            write(1001) zgaus
            write(1001) nfft2
         elseif (beam(1:12).eq.'gfftwavecircular') then
            write(1000,*) 'pola s  =',ss
            write(1000,*) 'theta   =',theta
            write(1000,*) 'phi     =',phi
            write(1000,*) 'pos x   =',xgaus
